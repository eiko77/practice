            ...
            switch ($_REQUEST['command']) {
                case 'insert':
                    if (!empty($_REQUEST['name'] & $_REQUEST['message'])) { //空のメッセージは禁止
                        //データ準備_
                        $sql1 = $pdo->prepare('insert into time_test values(null, ?, null, ?, 0)');
                        // 実行_データベースにデータ入る
                        $sql1->execute([$_REQUEST['name'],$_REQUEST['message']]);
                        //string $format, ?int $timestamp = null
                        //★リダイレクトの処理 いいねの数字が更新時増えないように
                        header("Location:http://localhost/~itsys/practice/timetest.php"); // 新しいページにリダイレクト
                ...
                case 'update':
                    ...
                    header("Location:http://localhost/~itsys/practice/timetest.php");
                    exit();

                    break;
                    //削除
                case 'delete':
                    $sql3  = $pdo->prepare('delete from time_test where number=?');
...
<?php
        //配列から取り出し
        foreach ($pdo->query('select*from time_test order by number desc') as $row) {
            //配列から出力した要素を利用して画面表示
            ...
            //echo date('Y年m月d日 H時i分s秒',$row['time']);
            echo '</div>';

            //削除ボタン
            echo '<div id="delete_btn">';
            echo '<form class="num" action="timetest.php" method ="post">';
            ...
            //更新　いいねボタンを押すと数字が増える
            echo '<form class="iine" action="timetest.php" method ="post">';
            echo '<input type="hidden" name="number" value="', $row['number'], '">';
